...
| Class | Purpose |
|---|---|
| `SampleAppApplication` | Spring Boot entry point with `@EnableRuntimeTests` |
| `Calculator` | `@Service` bean used as subject under test |
| `CalculatorTestSuite` | `@Component` + `@RuntimeTestSuite` — demonstrates Spring DI injection |
...
- **Auto-discovery** — suites are found by classpath scanning, no manual registration
