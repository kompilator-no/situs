 ...
 * {@link no.testframework.javalibrary.annotations.RuntimeTestSuite @RuntimeTestSuite}.
 *
 ...
 * <pre>{@code
 * Set<Class<?>> suites = ClasspathScanner.findRuntimeTestSuites("com.example.tests");
 * }</pre>
 *
...
public class ClasspathScanner {
     ...
     * {@code @RuntimeTestSuite}, without needing a package name.
     *
     ...
     */
    public static Set<Class<?>> findAllRuntimeTestSuites() {
            ...
            try {
                if ("file".equals(protocol)) {
                    ...
                    if (entry.isDirectory()) {
                        ...
                        candidates.stream()
                                .filter(c -> c.isAnnotationPresent(RuntimeTestSuite.class))
                                .peek(c -> log.info("Auto-discovered @RuntimeTestSuite: {}", c.getName()))
                                .forEach(found::add);
     ...
     * @return a set of matching classes; empty if none are found
     */
    public static Set<Class<?>> findRuntimeTestSuites(String basePackage) {
        Set<Class<?>> found = new HashSet<>();
        Set<Class<?>> candidates = findClasses(basePackage);
        for (Class<?> clazz : candidates) {
            if (clazz.isAnnotationPresent(RuntimeTestSuite.class)) {
                log.info("Auto-discovered @RuntimeTestSuite: {}", clazz.getName());
                found.add(clazz);
            }
        }
    ...
    /**
     * Scans all entries of an open {@link JarFile} for {@code @RuntimeTestSuite} classes.
     */
        ...
        while (entries.hasMoreElements()) {
            ...
            if (name.endsWith(".class") && !entry.isDirectory()) {
                ...
                try {
                    ...
                    log.debug("Skipping class '{}': {}", className, t.getMessage());
                }
            }
        }
    }
